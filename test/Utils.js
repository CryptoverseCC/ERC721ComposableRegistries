...
function safeTransferToAddress(from, registryAddress, params /* to, whichErc721, whichTokenId, data? */) {
    const safeTransferToAddressFunc = ERC721ComposableRegistry.abi.find(f => f.name === 'safeTransferToAddress' && f.inputs.length === params.length);
    const transferMethodTransactionData = web3Abi.encodeFunctionCall(safeTransferToAddressFunc, params);
    const txHash = web3.eth.sendTransaction({
...
module.exports.safeTransferToAddress = safeTransferToAddress;
