...
function safeTransferToAddress(from, registryAddress, to, whichErc721, whichTokenId) {
    const safeTransferToAddressFunc = ERC721ComposableRegistry.abi.find(f => f.name === 'safeTransferToAddress' && f.inputs.length === 3);
    const transferMethodTransactionData = web3Abi.encodeFunctionCall(
        safeTransferToAddressFunc, [to, whichErc721, whichTokenId]
    );
    const txHash = web3.eth.sendTransaction({
